<?php
if ( ( class_exists( 'LearnDash_Settings_Section' ) ) && ( !class_exists( 'LearnDash_Settings_Section_Translations_Learndash_2Checkout' ) ) ) {
	class LearnDash_Settings_Section_Translations_Learndash_2Checkout extends LearnDash_Settings_Section {

		// Must match the Text Domain
		private $project_slug = 'learndash-2checkout';
		...
		function __construct() {
			...
			$this->settings_section_label			=	__( 'LearnDash Gravity Forms', 'learndash-2checkout' );
			...
			if ( class_exists( 'LearnDash_Translations' ) ) {
				...
				if ( method_exists( 'LearnDash_Translations', 'register_translation_slug' ) ) {
					...
					LearnDash_Translations::register_translation_slug( $this->project_slug, LEARNDASH_2CHECKOUT_PLUGIN_PATH . 'languages' );
				} 
			} 
	...
	add_action( 'init', function() {
		LearnDash_Settings_Section_Translations_Learndash_2Checkout::add_section_instance();
	} );
}
